    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <!--放大镜效果-->
          <Zoom :skuInfos="skuImageList" />
          <!-- 小图列表 -->
          <ImageList :skuInfos="skuImageList" />
        </div>
        <!-- 右侧选择区域布局 -->
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="spuSaleAttr in spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                <dd
                  ...
                  :key="spu.id"
                  @click="getActive(spu,spuSaleAttr.spuSaleAttrValueList)"
                >
            ...
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model="skuNum" @change="getSkuNum"/>
                <a href="javascript:" class="plus" @click="skuNum++">+</a>
                <a href="javascript:" class="mins" @click="skuNum>1?skuNum--:1">-</a>
              </div>
              <div class="add">
                <a @click="getShopping">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  mounted() {
    this.$store.dispatch("getGoodsInfo", this.$route.params.kiuid);
  },
  computed: {
  ...
  methods: {
   ...
   async getShopping(){
     //async返回的是一个promise
      try {
        //本地存储与会话存储一般都存的字符串
       await this.$store.dispatch("getShopInfo",{skuid:this.$route.params.kiuid,skuNum:this.skuNum})
       sessionStorage.setItem("SHOPINFO",JSON.stringify(this.skuInfo))
